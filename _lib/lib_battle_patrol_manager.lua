...
PATROL_MANAGER_ATTACK_REORDER_INTERVAL = 5000;					-- when attacking a target, reissue the attack order at this interval
...
PATROL_MANAGER_DEFAULT_INTERCEPT_TIME = -1;
...
function patrol_manager:add_waypoint(new_dest, new_should_run, new_delay, new_orientation, new_width)
	...
	local BOOL_ScriptUnit_Input = false;
	...
	elseif is_scriptunits(new_dest) then
		BOOL_ScriptUnit_Input = true;
	...
	if self.is_debug or __patrol_manager_debug then
		...
		elseif BOOL_ScriptUnit_Input then
			self.bm:out(self.name .. " adding waypoint scriptunits " .. new_dest.name .. ", orient " .. new_orientation .. ", width " .. new_width .. ", delay " .. new_delay .. " and running " .. tostring(new_should_run));
		else
...
function patrol_manager:resume_patrol(reason)
	...
	if self.is_debug or __patrol_manager_debug then
		...
		elseif reason == PATROL_MANAGER_STARTING then
			self.bm:out(self.name .. " is starting - unique unit id is " .. tostring(self.sunit.unit:unique_ui_id()));
...
function patrol_manager:move_to_current_waypoint(force_run)
	...
	local BOOL_destination_is_sunits = false;
	
	-- "destination" here could be a vector or a unit, determine which
	local dest_vector = false;

	-- this is only used if the waypoint destination is a scriptunits collection
	local closest_standing_unit = false;
	
	if is_vector(destination) then
		BOOL_destination_is_vector = true;
		
		dest_vector = destination;
	elseif is_unit(destination) then
		dest_vector = destination:position();
	elseif is_scriptunits(destination) then
		BOOL_destination_is_sunits = true;
		closest_standing_unit = get_closest_standing_unit(destination, self.sunit.unit:position());

		-- test to see if we found a closest standing unit
		if closest_standing_unit then
			dest_vector = closest_standing_unit:position();
		end;
	end;
	...
	if self.waypoints[self.current_waypoint].calc_orient_at_runtime then
		-- guard against dest_vector being false
		if dest_vector then
	...
	if self.is_debug or __patrol_manager_debug then
		...
		elseif BOOL_destination_is_sunits then
			if dest_vector and closest_standing_unit then
				prepend_string = prepend_string .. "intercept closest unit with id " .. closest_standing_unit:unique_ui_id() .. " in scriptunits collection " .. destination.name .. " at " .. v_to_s(dest_vector);
			else
				prepend_string = prepend_string .. "intercept closest unit in scriptunits collection " .. destination.name .. " (but there are no standing units?)";
			end;
		else
	...
	if BOOL_destination_is_vector then
		...
		arrival_test = function() return is_routing_or_dead(destination) or self.sunit.unit:position():distance_xz(destination) < self.waypoint_reached_threshold end;
		
	elseif BOOL_destination_is_sunits then	
		movement_instruction = function()
			-- dest_vector could be nil, if no non-routing target unit exists
			if dest_vector then
				self.sunit.uc:goto_location_angle_width(dest_vector, dest_orientation, dest_width, dest_run);
			end;
			
			self.bm:repeat_callback(
				function()
					local closest_standing_unit = get_closest_standing_unit(destination, self.sunit.unit:position());
					if closest_standing_unit then
						dest_vector = closest_standing_unit:position();
						if self.is_debug or __patrol_manager_debug then
							self.bm:out(self.name .. " is reacquiring closest target while patrolling towards scriptunits " .. destination.name .. " - found unit with id " .. closest_standing_unit:unique_ui_id() .. " at position " .. v_to_s(dest_vector) .. ", moving there");
						end;
						self.sunit.uc:goto_location_angle_width(dest_vector, dest_orientation, dest_width, dest_run);
					else
						if self.is_debug or __patrol_manager_debug then
							self.bm:out(self.name .. " is reacquiring closest target while patrolling towards scriptunits " .. destination.name .. " but no standing unit in this collection could be found");
						end;
						dest_vector = false;
					end;
				end, 
		...
		arrival_test = function()
			-- return true if we don't have a destination, or if our distance to the destination is less than the waypoint-reached threshold
			return not dest_vector or self.sunit.unit:position():distance_xz(dest_vector) < self.waypoint_reached_threshold;
		end;
	else
		movement_instruction = function()
...
function patrol_manager:is_in_range_of_patrol_path_segment(range)
	...
	elseif is_scriptunits(dest_pos) then
		return true;
	end;
...
function patrol_manager:intercept()
	...
	-- debug output
	if self.is_debug or __patrol_manager_debug then
		self.bm:out(self.name .. " intercept triggered :: attacking unit with name " .. closest_unit:name() .. " and id " .. closest_unit:unique_ui_id() .. " at position " .. v_to_s(closest_unit:position()));
	end;
	...
	-- reissue the order every five seconds
	self.bm:repeat_callback(
		function()
			local closest_unit = get_closest_standing_unit(self.enemy_armies, self.sunit.unit:position());
			if not is_unit(closest_unit) then
			...
			if self.is_debug or __patrol_manager_debug then
				self.bm:out(self.name .. " intercept is re-issuing attack order to attack closest unit with name " .. closest_unit:name() .. " and id " .. closest_unit:unique_ui_id() .. " at position " .. v_to_s(closest_unit:position()));
			end;

			self.sunit.uc:attack_unit(closest_unit, true, true);
		end,
		PATROL_MANAGER_ATTACK_REORDER_INTERVAL,
		self.name
	);
