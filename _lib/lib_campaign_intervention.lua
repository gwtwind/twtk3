...
function intervention_manager:cache_camera_position()
	...
	self.cached_camera_x, self.cached_camera_y, self.cached_camera_d, self.cached_camera_b, self.cached_camera_h = cm:get_camera_position();
end;
...
function intervention_manager:camera_has_moved_from_cached()
	...
	return math.abs(self.cached_camera_x - x) > 1 or 
		math.abs(self.cached_camera_y - y) > 1 or
		math.abs(self.cached_camera_d - d) > 1 or
		math.abs(self.cached_camera_b - b) > 1 or
		math.abs(self.cached_camera_h - h) > 1;
end;


function intervention_manager:reset_cached_camera_position()
	self.camera_cache_position_set = false;
	self.cached_camera_x = -1;
	self.cached_camera_y = -1;
	self.cached_camera_d = -1;
	self.cached_camera_b = -1;
	self.cached_camera_h = -1;
end;
...
function intervention_manager:register_intervention(intervention)
	table.insert(self.intervention_list, intervention);	
end;


function intervention_manager:get_intervention(name)
	local intervention_list = self.intervention_list;
	
	for i = 1, #intervention_list do
		local current_intervention = intervention_list[i];
...
function intervention_manager:sort_interventions()
	...
	for i = 1, #current_eligible_interventions do
		...
		if (not fullscreen_panel_open or not current_intervention.wait_for_fullscreen_panel_dismissed) and
			...
			-- otherwise, if this intervention is set to not lock the ui, add it to the ui-unlocked intervention list, which goes at the end of the sorted list of eligible interventions we pass back
		...
		-- put the intervention in the correct place in the target list
...
function intervention_manager:event_triggered(event_name, context)
	...
	if intervention_listeners then
		...
		for i = 1, #copied_intervention_list do
			...
			if not cm:is_advice_enabled() and not current_intervention.allow_when_advice_disabled then
				if self.intervention_system_debug then
			...
			if not current_intervention:passes_precondition_check() then
				if self.intervention_system_debug then
			...
			elseif current_intervention.is_enqueued_for_triggering then
				if self.intervention_system_debug then
			...
			elseif current_intervention:get_min_turn() + self.intervention_system_last_advice_reset - 1 > cm:turn_number() then
				if self.intervention_system_debug then
					self:out("\t\tadvice was last reset on turn [" .. self.intervention_system_last_advice_reset .. "], this intervention can't play for [" .. current_intervention:get_min_turn() .. "] turns afterwards (inclusive of the turn it was reset), and it's only turn [" .. cm:turn_number() .. "], skipping");
				end;
	...
	if self.intervention_system_debug then
		self:out("about to call start_next_intervention()...");
		self:dec_tab();
		self:out(separator_str);
	end;
	
	-- attempt to start next intervention
	cm:wait_for_model_sp(function() self:start_next_intervention() end);
end;
...
function intervention_manager:start_next_intervention(force_no_wait, intervention_just_completed)
	local uim = self.uim;
	...
	if self.intervention_currently_active and not intervention_just_completed then
		if self.intervention_system_debug then
			self:out("");
			self:out(separator_str);
			self:out(separator_str);
			self:out("intervention_manager:start_next_intervention() is not starting next intervention as one is currently active");
			self:out(separator_str);
			self:out(separator_str);
		end;
		return;
	...
	if core:is_loading_screen_visible() then
		...
		core:progress_on_loading_screen_dismissed(function() self:start_next_intervention(force_no_wait, intervention_just_completed) end);
		return;		
	end;
	...
	--
	-- if there are any fullscreen panels being shown, including the event panel, then lock the ui (so that the player can only dismiss them) and wait for them to be dismissed
	--
	...
	if next_intervention.wait_for_fullscreen_panel_dismissed then
		...
		if open_fullscreen_panel then
			...
			cm:progress_on_fullscreen_panel_dismissed(
				function()					
					-- wait a little for the UI to be notified that the panel has closed
					cm:callback(
						function() 
							if self.intervention_system_debug then
								self:out("");
...
function intervention_manager:start_context_change_monitor()
	...
	core:add_listener(
		"intervention_manager_context_change_monitor",
		"ScriptEventPlayerBattleStartedSP",
		true,
		function()
	...
	local character_list = cm:query_local_faction():character_list();
...
function intervention_manager:lock_ui(value)
	...
	local modify_faction = cm:modify_faction(cm:get_local_faction());
	...
	if value then
		if not self.has_locked_ui then
			...
			modify_faction:disable_movement();
		...
		if self.has_locked_ui then
			...
			modify_faction:enable_movement();
...
function intervention_manager:state_to_string()
	...
	for i = 1, #intervention_list do
		str = str .. intervention_list[i]:state_to_string();
	end;
...
function intervention_manager:intervention_state_from_string(str)
	...
	local intervention = self:get_intervention(intervention_name);
			
	if not intervention then
		script_error("ERROR: intervention_state_from_string() is attempting to set up an intervention with name [" .. tostring(intervention_name) .. "] but it isn't registered. All interventions should be registered before the first tick.");
		return false;
	end;
...
function intervention_manager:advice_history_cleared()
	...
	-- stop all running interventions
	for i = 1, #intervention_list do
		local current_intervention = intervention_list[i];
	...
	-- start all interventions that are eligible
	for i = 1, #intervention_list do
		local current_intervention = intervention_list[i];
...
--- @desc Campaign interventions offer a mechanism for locking the UI and the progression of the game while the script takes the player through a sequence of events. This sequence can be as short as a few seconds to trigger some advice up to a full mid-game cinematic or a full scripted tour of a feature.
--- @desc An intervention is constructed with a trigger function, a priority cost, and one or more conditions that tell it when to trigger. The intervention is first started when @intervention:start is called, after which it monitors its trigger conditions. Once one of these conditions is met the intervention is enqueued for triggering, and its priority cost is considered alongside that of any other interventions that also wish to trigger at this time (if any). The intervention that is highest priority gets to trigger first - its cost is added to a cumulative cost counter, and other interventions are queued up behind it. Once the first intervention has finished the second is started, and so on, until the list of interventions that wish to trigger is exhausted or the cumulative cost exceeds a certain value (100). The trigger monitors of any interventions that didn't get to trigger are restarted so they can trigger again in future.
...
--
--- @section Usage
--- @desc An intervention may be created with @intervention:new, and optionally configured with a variety of settings (see @"intervention:Configuration"). Once created, one or more trigger conditions can be added with @intervention:add_trigger_condition.
--- @desc Preconditions may also be added to an intervention with @intervention:add_precondition. Preconditions are tests the intervention must pass before it's allowed to start. They are also checked prior to triggering.
--- @desc Once the intervention is set up it may be started with @intervention:start. This only needs to happen once during the course of a campaign game, for an intervention that was previously started will be restarted should the script save and load. This includes interventions that have triggered and shut down, so the preconditions of each intervention must be set up to ensure that it doesn't start back up on script load unless that is desired.
--- @desc Once an intervention triggers, it is imperative that it finishes by calling @intervention:complete (or @intervention:cancel), as until it does so the game is locked. Some functions are made available that wrap common intervention templates - see the functions list in the @"intervention:Intervention Behaviour Wrappers" section. These will automatically end the intervention at the appropriate time.
--
--- @section Output
--- @desc The intervention system outputs extensively to the Lua - Interventions tab.
----------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------
...
intervention = {
	...
	must_trigger_immediately = false,						-- this intervention will trigger as soon as its trigger conditions pass, regardless of what other interventions are enqueued. Only if an intervention is currently active/playing will this intervention not be triggered immediately
	precondition_list = {},
	trigger_conditions = {},
...
function intervention:new(name, cost, callback, is_debug)
	...
	if im:get_intervention(name) then
		script_error("ERROR: intervention:new() called but intervention with supplied name [" .. tostring(name) .. "] already exists");
		return false;
	end;
...
--- @desc Adds a restart event and conditional check. Should the event be received and the conditional check return true, then the intervention will restart.
--- @p string event name, Event name to listen for.
...
--- @desc If a turn countdown restart number is set, the intervention will attempt to restart the given number of turns after stopping. This countdown also applies if the intervention fails to start.
--- @p number turns
...
function intervention:take_priority_over_intervention(intervention_name)
	if not is_string(intervention_name) then
		script_error("ERROR: take_priority_over_intervention() called but supplied intervention name [" .. tostring(intervention_name) .. "] is not a string");
		return false;
	end;
	
	if intervention_name == self.name then
		script_error("ERROR: take_priority_over_intervention() called but supplied intervention name [" .. intervention_name .. "] is the name of this intervention");
		return false;
	end;
	
	if self.have_shared_turn_countdown_interventions then
		script_error("ERROR: take_priority_over_intervention() called but this intervention is shared - interventions don't support sharing and priority ordering");
		return false;
	end;
...
function intervention:give_priority_to_intervention(intervention_name, should_notify)
	if not is_string(intervention_name) then
		script_error("ERROR: give_priority_to_intervention() called but supplied intervention name [" .. tostring(intervention_name) .. "] is not a string");
		return false;
	end;
	
	if intervention_name == self.name then
		script_error("ERROR: give_priority_to_intervention() called but supplied intervention name [" .. intervention_name .. "] is the name of this intervention");
		return false;
	end;
	
	if self.have_shared_turn_countdown_interventions then
		script_error("ERROR: give_priority_to_intervention() called but this intervention is shared - interventions don't support sharing and priority ordering");
		return false;
	end;
...
function intervention:start(is_restarting, starting_from_other_intervention)
	...
	for intervention_name in pairs(self.give_priority_to_interventions_list) do
		local intervention = im:get_intervention(intervention_name);
	...
	if self.turn_countdown_restart > 0 then
		...
		if most_recent_intervention_trigger + self.turn_countdown_restart > cm:turn_number() then
			...
			self:start_turn_countdown(cm:turn_number() - most_recent_intervention_trigger);			-- compensate for how long ago the most recent trigger was
...
function intervention:trigger()
	...
	self.is_active = true;
	self.turn_last_triggered = cm:turn_number();
	self.callback();
end;
...
function intervention:start_turn_countdown(modifier)
	if self.turn_countdown_restart >= 0 then
		...
		cm:add_turn_countdown_event(cm:get_local_faction(), self.turn_countdown_restart - modifier, "ScriptEventInterventionTurnCountdownCompleted", self.name);
	end;
end;
...
function intervention:scroll_camera_for_intervention_action(targ_region, targ_x, targ_y, advice_key, infotext, mm, duration, scroll_complete_callback, continuation_callback)
	...
	if targ_region then
		...
		cutscene:action(function() cm:make_region_visible_in_shroud(cm:get_local_faction(), targ_region) end, 0);
	end;
...
--- @function trigger_ui_context_for_duration
--- @desc Allows the intervention to fire script context values to alter the UI (such as flashing an element, or showing a panel).
--- @p string context key, The string setup within the UI layout to affect.
--- @p string enable_value, The value to set when we enable.
--- @p string disable_value, The value to set when we disable.
--- @p number duration, How long to show this for.
function intervention:trigger_ui_context_for_duration(context_key, enable_value, disable_value, duration)

	if not context_key or not is_string(context_key) then
		script_error("trigger_ui_context_for_duration(): Invalid context_key passed. Must be a string and not nil.");
		return;
	end;
		
	if cm:is_multiplayer() then
		out.interventions( "trigger_ui_context_for_duration: Not enabling ui context as we're in an MP Game. Key: " .. context_key );
		return;
	end;

	if not enable_value or ( not is_string(enable_value) and not is_number(enable_value) ) then
		script_error("trigger_ui_context_for_duration(): We do not have an enable_value. It must be a number or a string.");
		return;
	end;

	if not disable_value or ( not is_string(disable_value) and not is_number(disable_value) ) then
		script_error("trigger_ui_context_for_duration(): We do not have an disable_value. It must be a number or a string.");
		return;
	end;

	if not duration or not is_number(duration) then
		script_error("trigger_ui_context_for_duration(): No duration passed in. Must be a number.");
		return;
	end;

	if duration <= 0 then
		script_error("trigger_ui_context_for_duration(): Duration must be a positive value.");
		return;
	end;
			...
			cm:callback(
				function() 
					if self.intervention_system_debug then
						self:out("");
						self:out(separator_str);
						self:out("trigger_ui_context_for_duration() :: Setting ui context " .. key .. " to " .. disable_value);
						self:out(separator_str);
					end;
	...
	if self.intervention_system_debug then
		self:out("");
		self:out(separator_str);
		self:out("trigger_ui_context_for_duration() :: Setting ui context " .. key .. " to " .. enable_value .. "for " .. tostring(duration) .. "seconds.");
		self:out(separator_str);
	end;
