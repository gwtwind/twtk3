...
function script_ai_planner:new(new_name, new_sunits, is_debug)
	...
	elseif is_scriptunit(new_sunits) then
		...
		if not is_table(new_sunits) then
			script_error("ERROR: tried to create script_ai_planner but supplied sunit list [" .. tostring(new_sunits) .. "] is not a table");
			return false;
		end;
...
function script_ai_planner:remove_sunits(input)
	...
	if not is_table(input) then
		script_error(self.name .. " ERROR: remove_sunits() called but supplied list " .. tostring(input) .. " is not a table or a sunit!");
		return false;
	end;
...
function script_ai_planner:move_to_force(enemy_force, reorder, defend_radius)
	...
	for i = 1, #enemy_force do
		...
		if is_scriptunit(curr_enemy_sunit) then
			if not is_routing_or_dead(curr_enemy_sunit.unit) and has_deployed(curr_enemy_sunit.unit) then
	...
	if not closest_enemy then
		if self.is_debug then
			self.bm:out(self.name .. ":" .. function_name .. " called but couldn't find any non-routing targets!");
		end;
		return;
	end;
	
	local reorder_function = function()
		self.bm:callback(function() self:move_to_force(enemy_force, true, defend_radius) end, self.move_to_force_reorder_interval, self.name);
	end;
	...
	if self.is_debug then
		if reorder then
			if should_defend then
				...
				self.bm:out(self.name .. " move_to_force() now targeting closest enemy " .. closest_enemy.name .. " at " .. v_to_s(closest_enemy.unit:position()));
			end;
		else
			if should_defend then
				...
				self.bm:out(self.name .. " move_to_force() called, targeting closest enemy " .. closest_enemy.name .. " at " .. v_to_s(closest_enemy.unit:position()));
			end;
		end;
	end;
...
function script_ai_planner:attack_force(enemy_force, reorder)
	...
	if is_routing_or_dead(self.sunit_list) then
		if self.is_debug then
			self.bm:out(self.name .. ": attack_force() called but all controlled units are routing, doing nothing");
			return;
		end;
	end;
	...
	if not closest_enemy then
		...
		if not non_routing_enemy_found then
			if self.is_debug then
				self.bm:out(self.name .. ":attack_force() called but couldn't find any targets that are not routing - exiting");
			end;			
		else	
			if self.is_debug then
				self.bm:out(self.name .. ":attack_force() called but couldn't find any targets that are visible and not routing - will move to the enemy force instead, and try attacking again in 30 seconds");
		...
		if self.is_debug then
			self.bm:out(self.name .. " attack_force() now attacking closest target " .. closest_enemy.name .. " at " .. v_to_s(closest_enemy.unit:position()));
		end;
	...
	bm:watch(
		...
		function()
			if self.is_debug then
				self.bm:out(self.name .. ": attack_force() is stopping as all controlled units are routing");
			end;
			self.bm:remove_process(self.name);
		end,
		self.name
