...
--- @function v
--- @desc A shorthand method for creating a vector. Battle only.
--- @p number x position
--- @p number y position
--- @p number z position
--- @return vector
...
function distance_between_forces(a, b, standing_only)
	...
	if is_scriptunits(a) then
		for i = 1, a:count() do
			...
			local current_distance = distance_between_forces_test(current_obj.unit, b, standing_only)
	...
	elseif is_scriptunit(a) then
		local current_distance = distance_between_forces_test(a.unit, b, standing_only)
...
function has_deployed(obj)
	if is_unit(obj) then
		return obj:is_controllable() or is_routing_or_dead(obj);
		
	elseif is_scriptunit(obj) then
		return obj.unit:is_controllable() or is_routing_or_dead(obj);
